...
import { InviteClientTransaction, InviteServerTransaction, Levels, LoggerFactory, NonInviteClientTransaction, NonInviteServerTransaction, TransactionState } from "../../../lib/core/index.js";
...
const makeMockIncomingRequest = (method) => {
    const request = jasmine.createSpyObj("IncomingRequest", ["method", "viaBranch"]);
    request.method = method;
    request.viaBranch = "z9hG4bK" + Math.floor(Math.random() * 10000000);
    return request;
};
/** Mocked incoming response factory function. */
const makeMockIncomingResponse = (statusCode, toTag) => {
    const response = jasmine.createSpyObj("IncomingResponse", [
        "statusCode",
        "toTag",
        "getHeader"
    ]);
    ...
    response.getHeader.and.callFake((name) => {
        return `[${name}]`;
    });
    return response;
};
...
const makeMockOutgoingRequest = (ruri = defaultURI) => {
    const request = jasmine.createSpyObj("OutgoingRequest", [
        "callId",
        "cseq",
        "method",
        "ruri",
    ...
    request.getHeader.and.callFake((name) => {
    ...
    request.setViaHeader.and.callFake((branch, scheme = "WSS") => {
...
const makeTransactionUser = () => {
    const user = jasmine.createSpyObj("TransactionUser", [
        "loggerFactory",
        "onTransportError",
        "onStateChange"
    ]);
...
const makeClientTransactionUser = () => {
    const user = jasmine.createSpyObj("ClientTransactionUser", [
        "loggerFactory",
        "onRequestTimeout",
        "onTransportError",
        "onStateChange",
...
const makeServerTransactionUser = () => {
...
describe("Core Transactions", () => {
    ...
    const executeTransactionTests = (transactionFactory) => {
        let transport;
        let user;
        let transaction;
    ...
    describe("ClientTransactions", () => {
        const executeClientTransactionTests = (clientTransactionFactory) => {
            let request;
            let transport;
            let user;
            let transaction;
        ...
        describe("InviteClientTransaction", () => {
            ...
            const non2xxAckTests = (requestsSent) => {
                ...
                it("it MUST generate an ACK with Via that matches original request", () => {
                    expect(transport.send.calls.mostRecent().args[0]).toMatch(new RegExp(`${request.getHeader("via") ? request.getHeader("via") : "via undef"}`));
                });
            };
            beforeEach(() => {
                request = makeMockOutgoingRequest();
    ...
    describe("ServerTransactions", () => {
        const executeServerTransactionTests = (serverTransactionFactory) => {
            let request;
            let transport;
            let user;
            let transaction;
        ...
        describe("InviteServerTransaction", () => {
            ...
            const serverTransactionFactory = (r, t, u) => {
                return new InviteServerTransaction(r, t, u);
            };
            executeTransactionTests(transactionFactory);
