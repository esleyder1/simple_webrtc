import { C, constructOutgoingResponse, IncomingRequestMessage, IncomingResponseMessage, InviteClientTransaction, InviteUserAgentClient, Parser, SessionDialog, SignalingState, Timers, TransportError, UserAgentCore } from "../../../lib/core/index.js";
import { URI } from "../../../lib/grammar/index.js";
import { connectTransportToUA, makeMockOutgoingRequestDelegate, makeMockOutgoingSubscribeRequestDelegate, makeMockSessionDelegate, makeMockSubscriptionDelegate, makeMockTransport, makeMockUA, makeMockUserAgentCoreDelegate, makeUserAgentCoreConfigurationFromUserAgent } from "../../support/core/mocks.js";
...
describe("Core UserAgentCore", () => {
    ...
    let configurationAlice;
    let configurationBob;
    let coreAlice;
    let coreBob;
    let transportAlice;
    let transportBob;
    ...
    describe("Session Initiation", () => {
        describe("Alice sends Bob an INVITE and...", () => {
            ...
            describe("Transport Error occurs", () => {
                ...
                it("Alice's UAC sends an INVITE, then receives internal 503 Service Unavailable", () => {
                    ...
                    expect(delegate.onReject.calls.mostRecent().args[0].message.statusCode).toBe(503);
                });
            ...
            describe("Alice CANCELs the request", () => {
                ...
                it("Alice's UAC sends an INVITE, then CANCEL, then ACK", () => {
                    ...
                    expect(delegate.onReject.calls.mostRecent().args[0].message.statusCode).toBe(487);
                });
            ...
            describe("Bob redirects", () => {
                ...
                it("Alice's UAC sends an INVITE, then ACK", () => {
                    ...
                    expect(delegate.onTrying).toHaveBeenCalledTimes(1);
                    expect(delegate.onRedirect.calls.mostRecent().args[0].message.statusCode).toBe(302);
                });
            ...
            describe("Bob rejects (without delegate)", () => {
                ...
                it("Alice's UAC sends an INVITE then an ACK", () => {
                    ...
                    expect(delegate.onReject).toHaveBeenCalledTimes(1);
                    expect(delegate.onTrying).toHaveBeenCalledTimes(0);
                    expect(delegate.onReject.calls.mostRecent().args[0].message.statusCode).toBe(486);
                });
            ...
            describe("Bob progresses and accepts, Alice sends ACK with wrong CSeq", () => {
                ...
                it("Alice's UAC sends an INVITE, then retransmits wrong ACK (3x)", () => {
                    ...
                    expect(delegate.onReject).toHaveBeenCalledTimes(0);
                    expect(delegate.onTrying).toHaveBeenCalledTimes(1);
                    expect(delegate.onAccept.calls.mostRecent().args[0].message.statusCode).toBe(200);
                });
            ...
            describe("Bob progresses and accepts, but Alice never sends an ACK", () => {
                ...
                it("Bob's UAS sends 100 Trying, then 180 Ringing, 200 OK (11x), times out, and sends BYE", () => {
                    ...
                    expect(transportBob.send.calls.all()[13].args[0]).toMatch(new RegExp(`^BYE ${uaAlice.contact.uri.toString()} SIP/2.0`));
                });
            });
        ...
        describe("Alice sends Bob an INVITE with Offer and...", () => {
            ...
            describe("Bob accepts with Answer in 200", () => {
                ...
                it("Alice's UAC sends an INVITE with Offer, then an ACK, and has stable session", () => {
                    ...
                    expect(transportAlice.send).toHaveBeenCalledTimes(2);
                    expect(transportAlice.send.calls.first().args[0]).toMatch(new RegExp(`^INVITE ${ruri} SIP/2.0`));
                    expect(transportAlice.send.calls.mostRecent().args[0]).toMatch(new RegExp(`^ACK ${uaBob.contact.uri.toString()} SIP/2.0`));
                    expect(delegate.onAccept).toHaveBeenCalledTimes(1);
                    expect(delegate.onProgress).toHaveBeenCalledTimes(1);
                    expect(delegate.onRedirect).toHaveBeenCalledTimes(0);
                    expect(delegate.onReject).toHaveBeenCalledTimes(0);
            ...
            describe("Bob accepts with Answer in reliable 183", () => {
                ...
                beforeEach((done) => {
                    ...
                    delegate.onProgress.and.callFake((response) => {
                        sessionAlice = response.session;
                        response.prack({
                ...
                it("Alice's UAC sends an INVITE with Offer, then a PRACK and ACK, and has stable session", () => {
                    ...
                    expect(transportAlice.send).toHaveBeenCalledTimes(3);
                    expect(transportAlice.send.calls.all()[0].args[0]).toMatch(new RegExp(`^INVITE ${ruri} SIP/2.0`));
                    expect(transportAlice.send.calls.all()[1].args[0]).toMatch(new RegExp(`^PRACK ${uaBob.contact.uri.toString()} SIP/2.0`));
                    expect(transportAlice.send.calls.all()[2].args[0]).toMatch(new RegExp(`^ACK ${uaBob.contact.uri.toString()} SIP/2.0`));
                    expect(delegate.onAccept).toHaveBeenCalledTimes(1);
                    expect(delegate.onProgress).toHaveBeenCalledTimes(1);
                    expect(delegate.onRedirect).toHaveBeenCalledTimes(0);
                    expect(delegate.onReject).toHaveBeenCalledTimes(0);
        ...
        describe("Alice sends Bob an INVITE without an Offer and...", () => {
            ...
            describe("Bob accepts with Offer in 200", () => {
                let sessionAlice;
                let sessionAliceDelegate;
                let sessionBob;
                let sessionBobDelegate;
                beforeEach((done) => {
                    coreBob.delegate = {
                        onInvite: (incomingRequest) => {
                            // Automatically send 100 Trying to mirror current UA behavior
                ...
                describe("Alice sends an in dialog BYE", () => {
                    beforeEach((done) => {
                        sessionBobDelegate.onBye.and.callFake((incomingRequest) => {
                            incomingRequest.accept();
                        });
                        sessionAlice.bye();
                        setTimeout(() => done(), 10); // transport calls are async, so give it some time
                    ...
                    it("Alice's UAC sends a BYE", () => {
                        expect(transportAlice.send).toHaveBeenCalledTimes(3);
                        expect(transportAlice.send.calls.mostRecent().args[0]).toMatch(new RegExp(`^BYE ${uaBob.contact.uri.toString()} SIP/2.0`));
                    });
                ...
                describe("Bob sends an in dialog BYE", () => {
                    beforeEach((done) => {
                        sessionAliceDelegate.onBye.and.callFake((incomingRequest) => {
                            incomingRequest.accept();
                        });
                        sessionBob.bye();
                        setTimeout(() => done(), 10); // transport calls are async, so give it some time
                    ...
                    it("Bob's UAC sends a BYE", () => {
                        expect(transportBob.send).toHaveBeenCalledTimes(4);
                        expect(transportBob.send.calls.mostRecent().args[0]).toMatch(new RegExp(`^BYE ${uaAlice.contact.uri.toString()} SIP/2.0`));
                    });
                ...
                describe("Alice sends an in dialog INFO", () => {
                    beforeEach((done) => {
                        sessionBobDelegate.onInfo.and.callFake((incomingRequest) => {
                            incomingRequest.accept();
                        });
                        sessionAlice.info();
                        setTimeout(() => done(), 10); // transport calls are async, so give it some time
                    ...
                    it("Alice's UAC sends a INFO", () => {
                        expect(transportAlice.send).toHaveBeenCalledTimes(3);
                        expect(transportAlice.send.calls.mostRecent().args[0]).toMatch(new RegExp(`^INFO ${uaBob.contact.uri.toString()} SIP/2.0`));
                    });
                ...
                describe("Bob sends an in dialog INFO", () => {
                    beforeEach((done) => {
                        sessionAliceDelegate.onInfo.and.callFake((incomingRequest) => {
                            incomingRequest.accept();
                        });
                        sessionBob.info();
                        setTimeout(() => done(), 10); // transport calls are async, so give it some time
                    ...
                    it("Bob's UAC sends a INFO", () => {
                        expect(transportBob.send).toHaveBeenCalledTimes(4);
                        expect(transportBob.send.calls.mostRecent().args[0]).toMatch(new RegExp(`^INFO ${uaAlice.contact.uri.toString()} SIP/2.0`));
                    });
                ...
                describe("Alice sends an in dialog INVITE (re-INVITE)", () => {
                    beforeEach((done) => {
                        sessionBobDelegate.onInvite.and.callFake((incomingRequest) => {
                            // Automatically send 100 Trying to mirror current UA behavior
                            incomingRequest.trying();
                            incomingRequest.accept({
                                statusCode: 200,
                    ...
                    it("Alice's UAC sends a INVITE and an ACK", () => {
                        ...
                        expect(transportAlice.send.calls.all()[2].args[0]).toMatch(new RegExp(`^INVITE ${uaBob.contact.uri.toString()} SIP/2.0`));
                        expect(transportAlice.send.calls.all()[3].args[0]).toMatch(new RegExp(`^ACK ${uaBob.contact.uri.toString()} SIP/2.0`));
                    });
                ...
                describe("Bob sends an in dialog INVITE (re-INVITE)", () => {
                    beforeEach((done) => {
                        sessionAliceDelegate.onInvite.and.callFake((incomingRequest) => {
                            // Automatically send 100 Trying to mirror current UA behavior
                            incomingRequest.trying();
                            incomingRequest.accept({
                                statusCode: 200,
                    ...
                    it("Bob's UAC sends a INVITE and an ACK", () => {
                        expect(sessionBob.signalingState === SignalingState.Stable);
                        expect(transportBob.send).toHaveBeenCalledTimes(5);
                        expect(transportBob.send.calls.all()[3].args[0]).toMatch(new RegExp(`^INVITE ${uaAlice.contact.uri.toString()} SIP/2.0`));
                        expect(transportBob.send.calls.all()[4].args[0]).toMatch(new RegExp(`^ACK ${uaAlice.contact.uri.toString()} SIP/2.0`));
                    });
                ...
                describe("Alice and Bob send re-INVITE at same time (glare)", () => {
                    ...
                    it("Alice's UAC sends an INVITE which is rejected with 491 Request Pending", () => {
                        ...
                        expect(transportAlice.send.calls.all()[4].args[0]).toMatch(new RegExp(`^ACK ${uaBob.contact.uri.toString()} SIP/2.0`));
                    });
                    it("Bob's UAC sends an INVITE which is rejected with 491 Request Pending", () => {
                        ...
                        expect(transportBob.send.calls.all()[5].args[0]).toMatch(new RegExp(`^ACK ${uaAlice.contact.uri.toString()} SIP/2.0`));
                    });
                });
                describe("Alice sends an in dialog MESSAGE", () => {
                    beforeEach((done) => {
                        sessionBobDelegate.onMessage.and.callFake((incomingRequest) => {
                            incomingRequest.accept();
                        });
                    ...
                    it("Alice's UAC sends a MESSAGE", () => {
                        expect(transportAlice.send).toHaveBeenCalledTimes(3);
                        expect(transportAlice.send.calls.mostRecent().args[0]).toMatch(new RegExp(`^MESSAGE ${uaBob.contact.uri.toString()} SIP/2.0`));
                    });
                ...
                describe("Bob sends an in dialog MESSAGE", () => {
                    beforeEach((done) => {
                        sessionAliceDelegate.onMessage.and.callFake((incomingRequest) => {
                            incomingRequest.accept();
                        });
                        sessionBob.message();
                        setTimeout(() => done(), 10); // transport calls are async, so give it some time
                    ...
                    it("Bob's UAC sends a MESSAGE", () => {
                        expect(transportBob.send).toHaveBeenCalledTimes(4);
                        expect(transportBob.send.calls.mostRecent().args[0]).toMatch(new RegExp(`^MESSAGE ${uaAlice.contact.uri.toString()} SIP/2.0`));
                    });
                ...
                describe("Alice sends an in dialog NOTIFY", () => {
                    beforeEach((done) => {
                        sessionBobDelegate.onNotify.and.callFake((incomingRequest) => {
                            incomingRequest.accept();
                        });
                    ...
                    it("Alice's UAC sends a NOTIFY", () => {
                        expect(transportAlice.send).toHaveBeenCalledTimes(3);
                        expect(transportAlice.send.calls.mostRecent().args[0]).toMatch(new RegExp(`^NOTIFY ${uaBob.contact.uri.toString()} SIP/2.0`));
                    });
                ...
                describe("Bob sends an in dialog NOTIFY", () => {
                    beforeEach((done) => {
                        sessionAliceDelegate.onNotify.and.callFake((incomingRequest) => {
                            incomingRequest.accept();
                        });
                    ...
                    it("Bob's UAC sends a NOTIFY", () => {
                        expect(transportBob.send).toHaveBeenCalledTimes(4);
                        expect(transportBob.send.calls.mostRecent().args[0]).toMatch(new RegExp(`^NOTIFY ${uaAlice.contact.uri.toString()} SIP/2.0`));
                    });
                ...
                describe("Alice sends an in dialog PRACK", () => {
                    beforeEach((done) => {
                        sessionBobDelegate.onPrack.and.callFake((incomingRequest) => {
                            incomingRequest.accept();
                        });
                        sessionAlice.prack();
                        setTimeout(() => done(), 10); // transport calls are async, so give it some time
                    ...
                    it("Alice's UAC sends a PRACK", () => {
                        expect(transportAlice.send).toHaveBeenCalledTimes(3);
                        expect(transportAlice.send.calls.mostRecent().args[0]).toMatch(new RegExp(`^PRACK ${uaBob.contact.uri.toString()} SIP/2.0`));
                    });
                ...
                describe("Bob sends an in dialog PRACK", () => {
                    beforeEach((done) => {
                        sessionAliceDelegate.onPrack.and.callFake((incomingRequest) => {
                            incomingRequest.accept();
                        });
                        sessionBob.prack();
                        setTimeout(() => done(), 10); // transport calls are async, so give it some time
                    ...
                    it("Bob's UAC sends a PRACK", () => {
                        expect(transportBob.send).toHaveBeenCalledTimes(4);
                        expect(transportBob.send.calls.mostRecent().args[0]).toMatch(new RegExp(`^PRACK ${uaAlice.contact.uri.toString()} SIP/2.0`));
                    });
                ...
                describe("Alice sends an in dialog REFER", () => {
                    beforeEach((done) => {
                        sessionBobDelegate.onRefer.and.callFake((incomingRequest) => {
                            incomingRequest.accept();
                        });
                        sessionAlice.refer();
                        setTimeout(() => done(), 10); // transport calls are async, so give it some time
                    ...
                    it("Alice's UAC sends a REFER", () => {
                        expect(transportAlice.send).toHaveBeenCalledTimes(3);
                        expect(transportAlice.send.calls.mostRecent().args[0]).toMatch(new RegExp(`^REFER ${uaBob.contact.uri.toString()} SIP/2.0`));
                    });
                ...
                describe("Bob sends an in dialog REFER", () => {
                    beforeEach((done) => {
                        sessionAliceDelegate.onRefer.and.callFake((incomingRequest) => {
                            incomingRequest.accept();
                        });
                        sessionBob.refer();
                        setTimeout(() => done(), 10); // transport calls are async, so give it some time
                    ...
                    it("Bob's UAC sends a REFER", () => {
                        expect(transportBob.send).toHaveBeenCalledTimes(4);
                        expect(transportBob.send.calls.mostRecent().args[0]).toMatch(new RegExp(`^REFER ${uaAlice.contact.uri.toString()} SIP/2.0`));
                    });
        ...
        describe("Alice sends Bob an PUBLISH and...", () => {
            ...
            describe("Bob rejects (without delegate)", () => {
                ...
                it("Alice's UAC sends an PUBLISH", () => {
                    ...
                    expect(delegate.onReject).toHaveBeenCalledTimes(1);
                    expect(delegate.onTrying).toHaveBeenCalledTimes(0);
                    expect(delegate.onReject.calls.mostRecent().args[0].message.statusCode).toBe(405);
                });
            });
        });
    });
        ...
        describe("Alice sends a SUBSCRIBE", () => {
            ...
            describe("to Bob", () => {
                ...
                it("Alice's UAC sends an SUBSCRIBE and receives rejection from Bob", () => {
                    ...
                    expect(delegate.onReject).toHaveBeenCalledTimes(1);
                    expect(delegate.onTrying).toHaveBeenCalledTimes(0);
                    expect(delegate.onReject.calls.mostRecent().args[0].message.statusCode).toBe(480);
                });
            });
            describe("to a presence server mocked up by Bob", () => {
                let subscriptionAlice;
                let subscriptionAliceDelegate;
                let subscriptionAliceMessage;
                beforeEach((done) => {
                    // eslint-disable-next-line @typescript-eslint/no-unused-vars
                    transportAlice.send.and.callFake((msg) => {
                    ...
                    delegate.onNotify.and.callFake((incomingRequest) => {
                    ...
                    const incomingResponseMessage = Parser.parseMessage(outgoingResponse.message, uaAlice.getLogger("sip.parser"));
                    ...
                    const outgoingRequestMessage1 = coreBob.makeOutgoingRequestMessage(C.NOTIFY, ruriNotify, fromNotify, toNotify, {
                        cseq: 1,
                        callId: incomingResponseMessage.callId,
                        toTag: incomingResponseMessage.fromTag,
                        fromTag: incomingResponseMessage.toTag
                    }, extraHeadersNotify, undefined);
                ...
                it("Alice's UAC sends an SUBSCRIBE and receives a 200 OK and a NOTIFY", () => {
                    ...
                    expect(delegate.onReject).toHaveBeenCalledTimes(0);
                    expect(delegate.onTrying).toHaveBeenCalledTimes(0);
                    expect(delegate.onAccept.calls.first().args[0].message.statusCode).toBe(200);
                });
                describe("which sends a couple of NOTIFY messages", () => {
                    let outgoingRequestMessage2;
                    let outgoingRequestMessage3;
                    beforeEach((done) => {
                        // Cobble together a NOTIFY and send it from Bob's core.
                        if (!(uaBob.configuration.uri instanceof URI)) {
                            throw new Error("uri not instance of URI");
                        ...
                        outgoingRequestMessage2 = coreBob.makeOutgoingRequestMessage(C.NOTIFY, ruri, fromNotify, toNotify, {
                            cseq: 2,
                            callId: subscriptionAliceMessage.callId,
                            toTag: subscriptionAliceMessage.toTag,
                            fromTag: subscriptionAliceMessage.fromTag
                        }, extraHeaders, undefined);
                        outgoingRequestMessage3 = coreBob.makeOutgoingRequestMessage(C.NOTIFY, ruri, fromNotify, toNotify, {
                            cseq: 3,
                            callId: subscriptionAliceMessage.callId,
                            toTag: subscriptionAliceMessage.toTag,
                            fromTag: subscriptionAliceMessage.fromTag
                    ...
                    describe("followed by a couple of broken NOTIFY messages (bad sequence numbers)", () => {
                        ...
                        it("Alice's UAS receives the NOTIFY messages and rejects them", () => {
                            ...
                            expect(transportAlice.send.calls.all()[4].args[0]).toMatch(new RegExp(`^SIP/2.0 500 Internal Server Error`));
                            expect(transportAlice.send.calls.all()[5].args[0]).toMatch(new RegExp(`^SIP/2.0 500 Internal Server Error`));
                            expect(delegate.onAccept).toHaveBeenCalledTimes(1);
                            expect(delegate.onProgress).toHaveBeenCalledTimes(0);
                            expect(delegate.onRedirect).toHaveBeenCalledTimes(0);
                            expect(delegate.onReject).toHaveBeenCalledTimes(0);
    ...
    describe("Alice handling Bad Requests", () => {
        ...
        describe("upon receiving a request with unsupported ruri scheme", () => {
            ...
            it("Alice's UAS rejects with 416 Unsupported URI Scheme", () => {
                expect(transportAlice.send).toHaveBeenCalledTimes(1);
                expect(transportAlice.send.calls.mostRecent().args[0]).toMatch(new RegExp(`^SIP/2.0 416 Unsupported URI Scheme`));
            });
        });
        // https://tools.ietf.org/html/rfc3261#section-8.2.2.1
