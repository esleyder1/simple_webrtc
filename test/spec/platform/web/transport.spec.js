...
import { makeEmitterSpy } from "../../../support/api/emitter-spy.js";
...
describe("Web Transport", () => {
    ...
    let mockServer;
    let mockServerWebSocket;
    let mockServerReceivedMessage;
    let transport;
    let transportStateSpy;
    let connectPromise;
    ...
    function initServer() {
        ...
        mockServer = new Server(server, { selectProtocol: (protocols) => "sip" });
        mockServer.on("connection", (socket) => {
            logger.log("Mock WebSocket Server: incoming connection");
            mockServerWebSocket = socket; // TODO: Client and WebSocket do not have same signature - close() is an issue.
     ...
     */
    function traverseTransportStateMachine(cycles = 2, cyclesCompleted = 0) {
    ...
    function transportSuiteIn(state) {
        ...
        describe(`callbacks fail calling .connect`, () => {
            function assertLoopDetected(error) {
            ...
            beforeEach(() => {
                onConnectMock.and.callFake(() => transport.connect().catch((error) => assertLoopDetected(error)));
                onDisconnectMock.and.callFake(() => transport.connect().catch((error) => assertLoopDetected(error)));
                onMessageMock.and.callFake(() => transport.connect().catch((error) => assertLoopDetected(error)));
            });
            afterEach(() => {
        ...
        describe(`callbacks fail calling .disconnect`, () => {
            ...
            beforeEach(() => {
                onConnectMock.and.callFake(() => transport.disconnect().catch((error) => assertLoopDetected(error)));
                onDisconnectMock.and.callFake(() => transport.disconnect().catch((error) => assertLoopDetected(error)));
                onMessageMock.and.callFake(() => transport.disconnect().catch((error) => assertLoopDetected(error)));
            });
            afterEach(() => {
                onConnectMock.and.stub();
                onDisconnectMock.and.stub();
    ...
    function connectIn(state) {
        let connectError;
        beforeEach(() => {
            resetAll();
            connectPromise = transport.connect().catch((error) => {
                connectError = error;
            });
        });
        switch (state) {
    ...
    function connectDisconnectIn(state) {
        let connectError;
        let disconnectError;
        beforeEach(() => {
            resetAll();
            ...
            disconnectPromise = transport.disconnect().catch((error) => {
    ...
    function connectDisconnectCompletesIn(state) {
        let connectError;
        let disconnectError;
        beforeEach(async () => {
            resetAll();
    ...
    function connectRejectedCompletesIn(state) {
        let connectError;
        beforeEach(async () => {
            resetAll();
            // Server rejects connection
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            mockServer.options = { selectProtocol: (protocols) => "invalid" };
            connectPromise = transport.connect().catch((error) => {
                connectError = error;
            });
            await soon(serverDelay);
    ...
    function connectTimeoutCompletesIn(state) {
        let openBlocked;
        let connectError;
        beforeEach(async () => {
            openBlocked = false;
            ...
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            transport.onWebSocketOpen = (ev, ws) => {
                openBlocked = true;
                return;
            };
            connectPromise = transport.connect().catch((error) => {
    ...
    function connectAcceptedCompletesIn(state) {
        let connectError;
        beforeEach(async () => {
            resetAll();
            connectPromise = transport.connect().catch((error) => {
    ...
    function disconnectConnectIn(state) {
        let connectError;
        let disconnectError;
        beforeEach(() => {
            resetAll();
    ...
    function disconnectConnectCompletesIn(state) {
        let connectError;
        let disconnectError;
        beforeEach(async () => {
            resetAll();
    ...
    function disconnectAcceptedCompletesIn(state) {
        let disconnectError;
        beforeEach(async () => {
            resetAll();
            disconnectPromise = transport.disconnect().catch((error) => {
    ...
    function disconnectSuiteIn(state) {
    ...
    function sendIn(state) {
        let sendError;
        beforeEach(() => {
            resetAll();
            sendPromise = transport.send(sendMessage).catch((error) => {
                sendError = error;
            });
        });
        switch (state) {
    ...
    function sendCompletesIn(state) {
        let sendError;
        beforeEach(async () => {
            resetAll();
            sendPromise = transport.send(sendMessage).catch((error) => {
    ...
    function sendServerCompletesIn(state) {
        beforeEach(async () => {
            resetAll();
