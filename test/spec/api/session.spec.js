...
import { Inviter, SessionState } from "../../../lib/api/index.js";
import { SessionState as SessionDialogState, SignalingState, Timers } from "../../../lib/core/index.js";
import { makeEmitterSpy } from "../../support/api/emitter-spy.js";
import { TransportFake } from "../../support/api/transport-fake.js";
import { connectUserFake, makeUserFake } from "../../support/api/user-fake.js";
...
describe("API Session", () => {
    ...
    let inviterStateSpy;
    let invitation;
    let invitationStateSpy;
    const invitationDelegateMock = jasmine.createSpyObj("SessionDelegate", [
        "onAck",
        "onCancel"
    ]);
    const inviterRequestDelegateMock = jasmine.createSpyObj("OutgoingRequestDelegate", ["onAccept", "onProgress", "onRedirect", "onReject", "onTrying"]);
    function bobAccept() {
        beforeEach(async () => {
            resetSpies();
            invitation.accept();
    ...
    function bobAcceptTerminate(dropAcks) {
    ...
    function bobProgressReliable2x() {
        beforeEach(async () => {
            resetSpies();
            invitation.progress({ rel100: true });
    ...
    function bobTerminate(bye = true) {
        beforeEach(async () => {
            resetSpies();
    ...
    function bobTerminate2x(bye = true) {
        let threw;
        beforeEach(async () => {
            threw = false;
            resetSpies();
    ...
    function inviteSuite(inviteWithoutSdp) {
        ...
        describe("Alice invite()", () => {
            ...
            describe("Bob accept(), Alice cancel() - a network glare condition (200 wins)", () => {
                ...
                beforeEach(async () => {
                    ...
                    invitation.accept();
                    if (inviteWithoutSdp) {
                        return Promise.resolve().then(() => Promise.resolve().then(() => Promise.resolve().then(() => Promise.resolve().then(() => inviter.cancel()))));
                    }
                    else {
                        return Promise.resolve().then(() => Promise.resolve().then(() => Promise.resolve().then(() => Promise.resolve().then(() => Promise.resolve().then(() => Promise.resolve().then(() => inviter.cancel()))))));
                    }
                });
                if (inviteWithoutSdp) {
            ...
            describe("Bob accept(), 200 has no SDP - Invalid 200", () => {
                beforeEach(async () => {
                    resetSpies();
                    {
                        ...
                        const sdh = invitation.sessionDescriptionHandler;
                        // eslint-disable-next-line @typescript-eslint/no-explicit-any
                        sdh.getDescriptionUndefinedBodyOnce = true;
                    }
            ...
            describe("Bob accept(), 200 SDP set fails - SDH Error", () => {
                beforeEach(async () => {
                    resetSpies();
                    {
                        ...
                        // eslint-disable-next-line @typescript-eslint/no-explicit-any
                        if (typeof inviter.setupSessionDescriptionHandler !== "function") {
                            throw new Error("setupSessionDescriptionHandler() undefined.");
                        }
                        // eslint-disable-next-line @typescript-eslint/no-explicit-any
                        inviter.setupSessionDescriptionHandler();
                        if (!inviter.sessionDescriptionHandler) {
                            throw new Error("SDH undefined.");
                        }
                        // eslint-disable-next-line @typescript-eslint/no-explicit-any
                        inviter.sessionDescriptionHandler.setDescriptionRejectOnce = true;
                    }
                ...
                describe("Bob accept(), ACK has no SDP - Invalid ACK", () => {
                    beforeEach(async () => {
                        resetSpies();
                        {
                            ...
                            // eslint-disable-next-line @typescript-eslint/no-explicit-any
                            inviter.sessionDescriptionHandler.getDescriptionUndefinedBodyOnce = true;
                        }
                        return invitation.accept().then(() => bob.transport.waitReceived()); // ACK
                    });
                    it("his ua should send 200, BYE", () => {
                ...
                describe("Bob accept(), ACK SDP set fails - SDH Error", () => {
                    beforeEach(async () => {
                        resetSpies();
                        {
                            ...
                            // eslint-disable-next-line @typescript-eslint/no-explicit-any
                            invitation.sessionDescriptionHandler.setDescriptionRejectOnce = true;
                        }
                        return invitation
                            .accept()
    ...
    describe("Forking...", () => {
        let bob2;
        let invitation2;
        let invitationStateSpy2;
        function bobsAccept(answerInAck) {
        ...
        function inviteSuiteFork(inviteWithoutSdp) {
            it("her ua should send nothing", () => {
                expect(alice.transportSendSpy).not.toHaveBeenCalled();
            });
