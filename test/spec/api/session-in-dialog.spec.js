...
import { Inviter, RequestPendingError, SessionState } from "../../../lib/api/index.js";
import { URI } from "../../../lib/grammar/index.js";
import { SignalingState } from "../../../lib/core/index.js";
...
describe("API Session In-Dialog", () => {
    ...
    function reinviteAccepted(withoutSdp) {
        beforeEach(async () => {
            resetSpies();
    ...
    function reinviteAcceptedAuthenticated(withoutSdp) {
        beforeEach(async () => {
            resetSpies();
            invitation.delegate = undefined;
            {
    ...
    function reinviteAcceptedWithoutDescriptionFailure(withoutSdp) {
        beforeEach(async () => {
            resetSpies();
            {
    ...
    function reinviteAcceptedOfferAnswerFailure(withoutSdp) {
        beforeEach(async () => {
            resetSpies();
            invitation.delegate = undefined;
            const session = inviter;
            return session
                ...
                const sdh = sessionDescriptionHandler; // assumes a spy
    ...
    function reinviteInProgress(withoutSdp) {
        beforeEach(async () => {
            resetSpies();
            invitation.delegate = {
                onInvite: () => {
        ...
        it("her ua should reject an additional INVITE", () => {
            inviter.invite({ withoutSdp }).catch((error) => {
    ...
    function reinviteRejected(withoutSdp) {
        beforeEach(async () => {
            resetSpies();
            {
    ...
    function reinviteRejectedRollbackFailure(withoutSdp) {
        beforeEach(async () => {
            resetSpies();
            {
                // Setup hacky thing to cause a rejection once
    ...
    function reinviteSuite(withoutSdp) {
        ...
        describe("Alice invite() without sdp", () => {
            ...
            describe("Bob accept() ACK processing", () => {
                beforeEach(async () => {
                    ...
                    invitation.accept();
                    // eslint-disable-next-line @typescript-eslint/no-explicit-any
                    invitation.sessionDescriptionHandler.setDescriptionWaitOnce = true;
                    await alice.transport.waitSent(); // ACK
                });
                it("her session state should be `established`", () => {
                    expect(inviter.state).toBe(SessionState.Established);
        ...
        describe("Alice invite()", () => {
            ...
            describe("Bob accept()", () => {
                ...
                describe("Carol invite() with Replaces to Alice...", () => {
                    let carol;
                    let replacesInviter;
                    let replacesInvitation;
                    function resetSpies3() {
                        resetSpies();
