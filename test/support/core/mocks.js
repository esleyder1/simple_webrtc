import { SIPExtension, UserAgentRegisteredOptionTags } from "../../../lib/api/index.js";
import { DigestAuthentication, IncomingRequestMessage, IncomingResponseMessage, LoggerFactory, Parser } from "../../../lib/core/index.js";
import { URI } from "../../../lib/grammar/index.js";
import { createRandomToken } from "../../../lib/core/messages/utils.js";
export function connectTransportToUA(transport, ua) {
    transport.send.and.callFake((message) => {
...
export function connectTransportToUAFork(transport, ua1, ua2) {
    transport.send.and.callFake((message) => {
        ...
        const requestMatches = (incomingMessage, ua) => {
            ...
            if (!ruriMatches(ua.configuration.uri) &&
...
export function makeMockOutgoingRequestDelegate() {
    const delegate = jasmine.createSpyObj("OutgoingRequestDelegate", [
        "onAccept",
        "onProgress",
        "onRedirect",
        "onReject",
...
/** Mocked user agent core delegate factory function. */
export function makeMockOutgoingSubscribeRequestDelegate() {
    const delegate = jasmine.createSpyObj("OutgoingSubscribeRequestDelegate", ["onAccept", "onProgress", "onRedirect", "onReject", "onTrying", "onNotify", "onNotifyTimeout"]);
    return delegate;
}
/** Mocked session delegate factory function. */
export function makeMockSessionDelegate() {
    const delegate = jasmine.createSpyObj("SessionDelegate", [
        "onAck",
        "onAckTimeout",
        "onBye",
        "onInfo",
    ...
    delegate.onAck.and.callFake((request) => {
        return Promise.resolve();
    });
    return delegate;
}
/** Mocked subscription delegate factory function. */
export function makeMockSubscriptionDelegate() {
    const delegate = jasmine.createSpyObj("SubscriptionDelegate", ["onNotify"]);
    return delegate;
}
/** Mocked transport factory function. */
export function makeMockTransport() {
    const transport = jasmine.createSpyObj("Transport", ["send"]);
...
/** Mocked user agent core delegate factory function. */
export function makeMockUserAgentCoreDelegate() {
    const delegate = jasmine.createSpyObj("UserAgentCoreDelegate", [
        "onInvite",
        "onMessage",
        "onNotify",
        "onRefer"
    ]);
    delegate.onInvite.and.callFake((incomingRequest) => {
        incomingRequest.trying();
    });
    return delegate;
}
/** Mocked UA factory function. */
export function makeMockUA(user, domain, displayName, transport) {
    ...
    const ua = {
        ...
        transport,
        getLogger: (category, label) => log.getLogger(category, label),
...
export function makeUserAgentCoreConfigurationFromUserAgent(ua) {
    // supported options
